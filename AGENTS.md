...
`.github/workflows/release.yml`, which builds the macOS `.dmg`, an unsigned
Windows `.exe` installer, and unsigned Linux `.deb` / AppImage bundles and
attaches them to a GitHub Release. Don't ship by running a local script and
pasting a file somewhere; tag it and let CI produce the artifact.
